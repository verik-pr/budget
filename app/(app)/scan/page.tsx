...
type ScannedItem = {
  ...
  categoryName: string
  excluded: boolean
  contributor: string       // who paid (explicit when split is set)
                    ...
                    <div key={i} className={`rounded-2xl overflow-hidden transition-all ${item.excluded ? "opacity-30" : ""}`}>
                      ...
                      {!item.excluded && (
                        <>
                          ...
                          {/* Show what partner owes for this item */}
                          {item.sharedWith && (
                            <div className="bg-zinc-900 border-t border-zinc-700 px-4 py-2 flex justify-between">
                              <p className="text-xs text-zinc-500">{partner.label.split(" ")[0]} schuldet dir</p>
                              <p className="text-xs font-bold" style={{ color: partner.color }}>
                                CHF {(item.amount * (item.sharedRatio ?? 0)).toFixed(2)}
                              </p>
                            </div>
                          )}
                        </>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-zinc-950 border-t border-zinc-900 px-6 py-4 safe-bottom">
          <div className="max-w-lg mx-auto flex items-center gap-4">
            <div className="flex-1">
              ...
              <p className="text-zinc-500 text-xs">{activeCount} Posten ausgewählt</p>
            </div>
            <button onClick={handleSave} disabled={saving || activeCount === 0}
