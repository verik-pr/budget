  ...
  return (
    <div className="bg-card border border-rule shadow-card rounded-3xl overflow-hidden">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i}
          className={`flex items-center gap-4 px-5 py-4 ${i < count - 1 ? "border-b border-rule/60" : ""}`}>
...
export function SkeletonCard({ className = "" }: { className?: string }) {
  return (
    <div className={`bg-card border border-rule shadow-card rounded-3xl px-5 py-6 space-y-3 ${className}`}>
